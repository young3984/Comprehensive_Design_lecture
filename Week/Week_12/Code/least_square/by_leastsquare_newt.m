...
for ik=1:n
    ...
    C=newton_least(data(:,1:7));           % 최소자승법으로 보간함수 만들기, 이 때 m이 우리 가원하는 고차다항식의 차수
    D=newton_least(data(:,8:14));
    E=newton_least(data(:,15:21));
    
    y(:,1:7)=exp(C*x(:,1:7));          % 각 열에 최소자승법을 사용해 보간한 Z축과의 거리 
    y(:,14:20)=exp(D*x(:,14:20));          % 각 열에 최소자승법을 사용해 보간한 Z축과의 거리 
    y(:,21:41)=exp(D*x(:,21:41));
%     return
